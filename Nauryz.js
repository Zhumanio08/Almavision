...
const TIME_LABELS = ["08:00", "10:00", "12:00", "14:00", "16:00", "18:00", "20:00", "22:00", "00:00"];
...
function generateCurrentMetrics() {
    ...
    return { congestion, speed, cars, trafficIndex, publicLoad, aqi, pm25, pm10, no2, co2, noise };
}
...
function buildPromptForAI() {
    ...
    const morningAvg = (ts.congestion.slice(0,3).reduce((a,b)=>a+b,0)/3).toFixed(0);
    const eveningAvg = (ts.congestion.slice(5,7).reduce((a,b)=>a+b,0)/2).toFixed(0);
...
async function getAIAdvice() {
    ...
    // Проверяем API ключ
    if (!GROQ_API_KEY || GROQ_API_KEY === "gsk_ВАШ_КЛЮЧ_СЮДА") {
        container.innerHTML = `
            <div class="ai-advice-content">
                <div class="advice-header" style="color: #d32f2f;">⚠️ API key not configured</div>
                <div style="padding: 16px;">Insert your Groq API key in script.js</div>
            </div>
        `;
        return;
    }
    ...
    if (timeSinceLast < MIN_REQUEST_INTERVAL && lastRequestTime !== 0) {
        const waitSeconds = Math.ceil((MIN_REQUEST_INTERVAL - timeSinceLast) / 1000);
        container.innerHTML = `
            <div class="ai-advice-content">
    ...
    container.innerHTML = `
        <div class="ai-advice-content">
            <div class="advice-header">🤖 Groq AI analyzing data...</div>
            <div style="text-align: center; padding: 40px;">
                <div class="loading-spinner"></div>
                <p style="margin-top: 16px;">Requesting AI recommendations...</p>
            </div>
        </div>
    `;
    ...
    const requestBody = {
        ...
        messages: [
            {
                role: "system",
                content: "You are an expert urban management advisor for the mayor of Almaty. Answer in Russian language, be specific and actionable."
            },
            {
                role: "user",
                content: prompt
...
function renderKPI() {
    ...
    const items = [
        { label: "Пробники", value: `${m.congestion}%`, color: getColor(m.congestion, 65, 75) },
        { label: "Скорость потока", value: `${m.speed} km/h`, color: getColorInverse(m.speed, 35, 25) },
        { label: "Машины(тыс)", value: m.cars, color: getColor(m.cars, 10, 12) },
        { label: "Пробочный индекс", value: `${m.trafficIndex}/10`, color: getColor(m.trafficIndex, 5, 7) },
        { label: "Общественная нагрузка", value: `${m.publicLoad}%`, color: getColor(m.publicLoad, 75, 85) },
        { label: "AQI", value: m.aqi, color: getColor(m.aqi, 60, 85) },
        { label: "PM2.5", value: `${m.pm25} µg/m³`, color: getColor(m.pm25, 25, 35) },
        { label: "PM10", value: `${m.pm10} µg/m³`, color: getColor(m.pm10, 40, 50) },
        { label: "NO₂", value: `${m.no2} µg/m³`, color: getColor(m.no2, 30, 40) },
        { label: "CO₂", value: `${Math.round(m.co2)} ppm`, color: getColor(m.co2, 450, 500) },
        { label: "Шум (Дб)", value: `${Math.round(m.noise)} dB`, color: getColor(m.noise, 60, 70) }
    ];
...
function destroyCharts() {
    if (transportChart) { transportChart.destroy(); transportChart = null; }
    if (ecoChart) { ecoChart.destroy(); ecoChart = null; }
}
...
function renderCharts() {
    ...
    transportChart = new Chart(ctx1, {
        ...
        data: {
            labels: TIME_LABELS,
            datasets: [
                { label: 'Пробки (%)', data: timeSeries.congestion, borderColor: '#ed6c02', backgroundColor: 'rgba(237,108,2,0.1)', tension: 0.3, fill: true, yAxisID: 'y' },
                { label: 'Скорость потока (km/h)', data: timeSeries.speed, borderColor: '#2e7d32', backgroundColor: 'rgba(46,125,50,0.1)', tension: 0.3, fill: true, yAxisID: 'y1' }
            ]
        },
        options: {
            responsive: true,
            ...
            scales: { 
                y: { title: { display: true, text: 'Congestion (%)' }, min: 0, max: 100 }, 
                y1: { position: 'right', title: { text: 'Speed (km/h)' }, min: 0, max: 80, grid: { drawOnChartArea: false } }
            }
        }
    });
    ...
    ecoChart = new Chart(ctx2, {
        ...
        data: {
            labels: TIME_LABELS,
            datasets: [
                { label: 'AQI', data: timeSeries.aqi, borderColor: '#d32f2f', backgroundColor: 'rgba(211,47,47,0.1)', tension: 0.3, fill: true },
                { label: 'PM2.5 (µg/m³)', data: timeSeries.pm25, borderColor: '#1565c0', backgroundColor: 'rgba(21,101,192,0.1)', tension: 0.3, fill: true }
            ]
        },
        options: {
            responsive: true,
            maintainAspectRatio: true,
            scales: { y: { title: { display: true, text: 'Values' }, min: 0, max: 150 } }
        }
    });
}
...
function init() {
    ...
    document.getElementById('aiAdviceBtn')?.addEventListener('click', getAIAdvice);
}
